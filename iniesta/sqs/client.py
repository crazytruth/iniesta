...
from insanic.log import sqs_logger, sqs_error_logger
...
class SQSClient:
    ...
    def __init__(self, queue_name, queue_url, retry_count=1, lock_timeout=10 * 10):
    ...
    async def initialize(cls, *, queue_name, endpoint_url=None):
        ...
        try:
            async with session.create_client('sqs', endpoint_url=cls.endpoint_url) as client:
                response = await client.get_queue_url(QueueName=queue_name)
        except botocore.exceptions.ClientError as e:
            error_message = f"[{e.response['Error']['Code']}]: {e.response['Error']['Message']} {queue_name}"
            sqs_error_logger.critical(error_message)
            ...
            sqs_client = cls(queue_name, queue_url)

        # # check if subscription exists
        # service_subscriptions = None
        # async for subs in sns_client.list_subscriptions_by_topic():
        #     subscription_arn = subs['SubscriptionArn']
        #
        #     if subscription_arn.endswith(f"mmt-{settings.MMT_ENV}-{settings.SERVICE_NAME}"):
    ...
    async def handle_message(self, message):
        ...
        #     sqs_error_logger.critical(f"[SQS] {str(e)}")
        # except KeyError as e:
        #     # sqs_error_logger.critical(f"[SQS] Unknown handler for {sqs_message.event}")
        #
        #     raise
    ...
    def handle_error(self, exc):
        ...
        extra = {
            "insanic_event": message.event,  # TODO: get insanic event
        ...
        sqs_error_logger.critical(f"[SQS] Error while handling message: {str(exc)}", exc_info=exc, extra=extra)

    async def handle_success(self, client, message):

        # if success must delete message from sqs
        sqs_logger.info(f"[SQS] Message handled successfully: msg_id={message.message_id}")
        resp = await client.delete_message(
            QueueUrl=self.queue_url,
            ReceiptHandle=message.receipt_handle
        )
        sqs_logger.debug(f"[SQS] Message deleted: msg_id={message.message_id} "
    ...
    async def _poll(self):
        ...
        try:
            while self._loop.is_running() and self._receive_messages:
                try:
                    response = await client.receive_message(
                        QueueUrl = self.queue_url,
                        MaxNumberOfMessages = settings.SQS_RECEIVE_MESSAGE_MAX_NUMBER_OF_MESSAGES,
                        WaitTimeSeconds = settings.SQS_RECEIVE_MESSAGE_WAIT_TIME_SECONDS,
                        AttributeNames = ['All'],
                        MessageAttributeNames = ['All']
                    )
                except botocore.exceptions.ClientError as e:
                    sqs_error_logger.critical(f"[SQS] [{e.response['Error']['Code']}]: {e.response['Error']['Message']}")
                else:
            ...
            sqs_logger.info("[SQS] POLLING TASK CANCELLED")
            return "Cancelled"
        except StopPolling:
            # mainly used for tests
            sqs_logger.info("[SQS] STOP POLLING")
            return "Stopped"
        except Exception as e:
            if self._receive_messages and self._loop.is_running():
                sqs_error_logger.critical("[SQS] POLLING TASK RESTARTING")
                self._polling_task = self._loop.create_task(self._poll())
            sqs_error_logger.exception("[SQS] POLLING EXCEPTION CAUGHT")
        finally:
            await client.close()
