...
class TestSNSSQSIntegration(SQSInfra, SNSInfra):
    ...
    async def test_integration(self, start_local_aws, create_service_sqs, create_sqs_subscription, add_permissions,
                               sns_client, sqs_client, loop):
        ...
        try:
            message = sns_client.create_message(event="Request", message=json.dumps({"id": some_id}), round="one")
    ...
    async def test_filters(self, start_local_aws, create_sqs_subscription, sqs_client, sns_client, add_permissions):
        some_id = uuid.uuid4().hex
        received_messages = []
    ...
    async def test_delete_sqs_message(self, start_local_aws, create_sqs_subscription, add_permissions,
                                      sqs_client, sns_client, monkeypatch):
        ...
        publish_tasks = []
        for i in range(10):
            message = sns_client.create_message(event="Request", message=json.dumps({"id": i}), round=i)
    ...
    async def test_confirm_permissions(self, start_local_aws, create_sqs_subscription, add_permissions,
