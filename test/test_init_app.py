...
class TestInitializeWithSNS(SNSInfra):
    ...
    async def test_insanic_run_with_topic_but_no_queue(self, insanic_application, test_server, create_global_sns):
        with pytest.raises(botocore.exceptions.ClientError) as exc_info:
            await test_server(insanic_application)
    ...
    async def test_insanic_run_with_topic_and_queue_but_no_subscriptions(self, insanic_application, test_server,
                                                                         create_global_sns, create_service_sqs):
